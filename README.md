...
Standalone-версия страницы «Карта агентов» из платформы AdaptAI/RAG-system —
...
Извлечено как отдельное приложение для деплоя на Vercel. Бэкенд не нужен:
...
1. Запушить папку в репозиторий GitHub.
2. На vercel.com → New Project → импортировать репозиторий.
3. Framework автоматически определится как **Vite**
   (`build` → `npm run build`, output → `dist`). Настройки менять не нужно.

Либо через CLI из этой папки:
